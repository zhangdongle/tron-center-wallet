...
@RestController
@RequestMapping("/tron/block-height" )
public class TronBlockController extends BaseController {
     ...
     * 查询区块高度列表
     */
    @PreAuthorize("@ss.hasPermi('tron:block-height:list')")
    @GetMapping("/list")
     ...
     * 导出区块高度列表
     */
    @PreAuthorize("@ss.hasPermi('tron:block-height:export')" )
    ...
    public AjaxResult export(TronBlock tronBlock) {
        ...
        return util.exportExcel(list, "block-height" );
    }

    /**
     * 获取区块高度详细信息
     */
    @PreAuthorize("@ss.hasPermi('tron:block-height:query')" )
     ...
     * 新增区块高度
     */
    @PreAuthorize("@ss.hasPermi('tron:block-height:add')" )
     ...
     * 修改区块高度
     */
    @PreAuthorize("@ss.hasPermi('tron:block-height:edit')" )
     ...
     * 删除区块高度
     */
    @PreAuthorize("@ss.hasPermi('tron:block-height:remove')" )
